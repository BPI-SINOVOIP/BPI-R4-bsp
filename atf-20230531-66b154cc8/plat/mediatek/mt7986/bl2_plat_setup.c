...
static void mtk_wdt_init(int enable)
{
	...
	sta = mmio_read_32(MTK_WDT_STA);

	if (!sta) {
		NOTICE("WDT: Cold boot\n");
	} else {
		NOTICE("WDT: Reboot status is 0x%x:\n", sta);
		if (sta & WDT_REASON_HW_RST)
			NOTICE("     Watchdog timeout\n");
		if (sta & WDT_REASON_SW_RST)
			NOTICE("     Software reset (reboot)\n");
		if (sta & WDT_REASON_SECURE_RST)
			NOTICE("     Secure reset\n");
		if (sta & WDT_REASON_THERMAL_RST)
			NOTICE("     Thermal-triggered reset\n");
		if (sta & WDT_REASON_SPM_RST)
			NOTICE("     SPM-trigger reset\n");
		if (sta & WDT_REASON_SPM_THERMAL_RST)
